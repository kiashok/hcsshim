...
func newProcess(c *Container, spec *oci.Process, process runtime.Process, pid uint32, init bool) *containerProcess {
	...
	p.writersWg.Add(1)
	go func() {
		ctx, span := ot.StartSpan(context.Background(), "newProcess::waitBackground")
		defer span.End()
		span.SetAttributes(
			attribute.String(logfields.ContainerID, p.cid),
		...
		go func() {
			...
			_, span := ot.StartSpan(context.Background(), "newProcess::waitBackground::waitAllWaiters")
			defer span.End()
			span.SetAttributes(
...
func (p *containerProcess) Wait() (<-chan int, chan<- bool) {
	ctx, span := ot.StartSpan(context.Background(), "opengcs::containerProcess::Wait")
	span.SetAttributes(
		attribute.String("cid", p.cid),
		attribute.Int64("pid", int64(p.pid)))
...
func (ep *externalProcess) Wait() (<-chan int, chan<- bool) {
	_, span := ot.StartSpan(context.Background(), "opengcs::externalProcess::Wait")
