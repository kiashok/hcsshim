	...
	go func() {
		...
		for {
			if !b.hasQuitPending.Load() {
				...
				var span trace.Span
				if base.OpenTelemetrySpanContext != nil {
					spanContextConfig := trace.SpanContextConfig{}
					if bytes, err := hex.DecodeString(base.OpenTelemetrySpanContext.TraceID); err == nil {
						copy(spanContextConfig.TraceID[:], bytes)
					}
					if bytes, err := hex.DecodeString(base.OpenTelemetrySpanContext.SpanID); err == nil {
						copy(spanContextConfig.SpanID[:], bytes)
					}
					spanContextConfig.TraceFlags = trace.TraceFlags(base.OpenTelemetrySpanContext.TraceFlags)
					if base.OpenTelemetrySpanContext.TraceState != "" {
						if ts, err := trace.ParseTraceState(base.OpenTelemetrySpanContext.TraceState); err == nil {
							spanContextConfig.TraceState = ts
						}
					}
					sc := trace.NewSpanContext(spanContextConfig)
					ctx, span = ot.StartSpanWithRemoteParent(
						context.Background(),
