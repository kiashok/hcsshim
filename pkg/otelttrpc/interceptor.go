...
import (
	...
	skdtrace "go.opentelemetry.io/otel/sdk/trace"
...
const metadataTraceContextKey = "traceparent"

type options struct {
	sampler skdtrace.Sampler
}
...
// auto-created spans.
func WithSampler(sampler skdtrace.Sampler) Option {
...
func getParentSpanFromContext(ctx context.Context) (trace.SpanContext, bool) {
	...
	traceContextBinary, err := base64.StdEncoding.DecodeString(traceContext[0])
	if err != nil {
		return trace.SpanContext{}, false
	}
	carrier := propagation.MapCarrier{
		metadataTraceContextKey: string(traceContextBinary),
	}
...
func ClientInterceptor(opts ...Option) ttrpc.UnaryClientInterceptor {
	...
	return func(ctx context.Context, req *ttrpc.Request, resp *ttrpc.Response, info *ttrpc.UnaryClientInfo, inv ttrpc.Invoker) (err error) {
		...
		injectSpanContextIntoRequest(ctx, span.SpanContext(), req)
		return inv(ctx, req, resp)
	}
}

func injectSpanContextIntoRequest(ctx context.Context, sc trace.SpanContext, req *ttrpc.Request) {
	var propagator = propagation.TraceContext{}
	spanCtx := trace.ContextWithSpanContext(ctx, sc)
	carrier := propagation.MapCarrier{}
	propagator.Inject(spanCtx, carrier)

	if traceParent, ok := carrier[metadataTraceContextKey]; ok {
		b64 := base64.StdEncoding.EncodeToString([]byte(traceParent))
...
func ServerInterceptor(opts ...Option) ttrpc.UnaryServerInterceptor {
	...
	return func(ctx context.Context, unmarshal ttrpc.Unmarshaler, info *ttrpc.UnaryServerInfo, method ttrpc.Method) (_ interface{}, err error) {
		...
		if ok {
			ctx, span = ot.StartSpanWithRemoteParent(ctx, name, parent, opts...)
		} else {
			ctx, span = ot.StartSpan(ctx, name, opts...)
		}
		defer span.End()
		defer func() { setSpanStatus(span, err) }()
