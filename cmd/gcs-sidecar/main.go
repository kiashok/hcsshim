...
func main() {
	...
	gcsConn, err := winio.Dial(ctx, hvsockAddr)
	if err != nil {
	...
	go recvFromSidecarAndSendResponse(gcsConn)
	wg.Wait()
	///
	/*
...
func recvFromSidecarAndSendResponse(gcsConn *winio.HvsockConn) {
	...
	for {
		...
		str := string(buffer[:length])
		replyString := fmt.Sprintf("GCS: Received command %s", str)
		fmt.Printf("GCS: Received command %d\t:%s\n", length, str)
