...
func (cco ContainerdClientOptions) NewClient(
	...
	defOpts := []client.ClientOpt{
		client.WithDefaultNamespace(cco.Namespace),
	}
	opts = append(defOpts, opts...)
	c, err := client.NewWithConn(conn, opts...)
	if err != nil {
...
func GetImageChainID(ctx context.Context, tb testing.TB, client *client.Client, image, platform string) string {
	tb.Helper()
...
func CreateActiveSnapshot(ctx context.Context, tb testing.TB, client *client.Client, snapshotter, parent, key string, opts ...snapshots.Opt) []mount.Mount {
	tb.Helper()
...
func CreateViewSnapshot(ctx context.Context, tb testing.TB, client *client.Client, snapshotter, parent, key string, opts ...snapshots.Opt) []mount.Mount {
	tb.Helper()
...
func PullImage(ctx context.Context, tb testing.TB, client *client.Client, ref, plat string) string {
	tb.Helper()
	...
	opts := []client.RemoteOpt{
		client.WithPlatform(plat),
		client.WithPullUnpack,
	}

	if s, err := imagesutil.SnapshotterFromPlatform(plat); err == nil {
		opts = append(opts, client.WithPullSnapshotter(s))
	}
